...
| **Agent Bounty Guard** | Found a live bounty repository whose submission rules ask AI coding agents to paste their session context (system prompt, startup instructions, local paths, token budget) under rotating field names, while hidden comments tell agents to ignore a visible notice. Built a read-only, dependency-free detector that keys on the request itself: it blocked 183 of 184 live honeypot issues and produced no findings on 14 widely used repositories or 90 legitimate issues about system prompts. Each detection rule is mutation-checked. | [Case study](case-studies/AI_AGENT_BOUNTY_HONEYPOT_GUARD_2026-09-24.md) · [Machine receipt](evidence/portfolio/agent_bounty_honeypot_guard_2026-09-24.json) · [Source and tests](examples/agent_bounty_guard/) |
