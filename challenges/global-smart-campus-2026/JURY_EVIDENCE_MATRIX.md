...
- Regression suite after privacy + audit-chain + tenant-isolation hardening: **22 tests; all four standard CI platforms passed in GitHub Actions run 35930830681**.
- Tenant isolation: `mvp/access_control.py` validates tenant scopes and fails closed on cross-tenant access; audit-chain tests reject cross-tenant review actions, tenant-scope switching, and malformed tenant identifiers.
